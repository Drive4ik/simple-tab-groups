...
const SECTION_SEARCH = 'search',
    SECTION_PARENTS_LIST = 'parentsList',
...
export default {
    ...
    computed: {
        ...
        filteredParents() {
            let filteredParents = this.parents((gr) => {
                return !this.search || Utils.mySearchFunc(this.search.toLowerCase(), gr.title);
            });
    ...
    methods: {
        ...
        async createNewGroup(tabIds, proposalTitle, applyGroupWithTabId, parentId) {
            ...
            let newGroupWindowId = Cache.getWindowGroup(this.currentWindow.id) ? undefined : this.currentWindow.id,
                newGroup = await Groups.add(newGroupWindowId, tabIds, newGroupTitle, parentId);
        ...
        async applyGroup(group, tab, closePopup = false, openInNewWindow = false) {
        ...
        async openGroups(groups, openInNewWindow = false) {
            if (groups.length < 1) {
                return alert('No groups to open')
            }
            const promises = groups.filter(gr => !this.isOpenedGroup(gr)).map(gr => this.applyGroup(gr, {}, false, openInNewWindow));
    ...
    >
        <header class="p-indent">
            <div :class="['field', {'has-addons': searchDelay.length}]">
                ...
                <template v-if="searchDelay.length">
                    ...
                    <div class="control">
                        <button :class="['button', {'is-active': searchOnlyGroups}]"
                                @click="searchOnlyGroups = !searchOnlyGroups"
                                v-text="lang('searchOnlyGroups')"></button>
                    </div>
                </template>
            </div>
        </header>
        ...
        <main id="result" :class="['is-full-width', dragData ? 'drag-' + dragData.itemType : false]">
                        ...
                        <div v-if="filteredGroups.length" class="search-scrollable no-outline">
                            <div v-for="group in filteredGroups" :key="group.id" v-if="group.parentId === parent.id">
                                    ...
                                    >
                                        ...
                                        <div class="item-title clip-text">
                                            <span v-if="tab.container" :title="tab.container?.name"
                                                  :class="`size-16 userContext-icon identity-icon-${tab.container?.icon} identity-color-${tab.container?.color}`"></span>
                                ...
                                <template v-else>
                                    ...
                                    >
                                        <div class="item-icon">
                                            <img v-if="isTabLoading(tab)" src="/icons/refresh.svg"
                                                 class="spin size-16 align-text-bottom"/>
                                            <img v-else :src="tab.favIconUrl" class="size-16" loading="lazy"
                                                 decoding="async"/>
                                        </div>
                                        <div class="item-title clip-text">
                                    <span v-if="showMuteIconTab(tab)" @click.stop="toggleMuteTab(tab)"
                                        ...
                                        <img :src="tab.audible ? '/icons/audio.svg' : '/icons/audio-mute.svg'"
                                             class="size-16 align-text-bottom"/>
                                    </span>
                                            <span v-if="tab.container" :title="tab.container?.name"
                                                  :class="`size-16 userContext-icon identity-icon-${tab.container?.icon} identity-color-${tab.container?.color}`"></span>
                                            <span :class="{'tab-discarded': tab.discarded}"
                                                  v-text="getTabTitle(tab)"></span>
                                        </div>
                                        <div class="item-action flex-on-hover">
                                    <span class="size-16 cursor-pointer" @click.stop="removeTab(tab)"
                                          :title="lang('deleteTab')">
            ...
            <div v-if="section === SECTION_GROUPS_LIST">
                    ...
                    <div class="content expanded" :id="'content-'+parent.id">
                        ...
                        <div class="create-new-group">
                            <div class="group item" tabindex="0"
                                 @click="createNewGroup(undefined, undefined, true, parent.id)"
                                 @keydown.enter="createNewGroup(undefined, undefined, true, parent.id)"
                ...
                <div class="create-new-group">
                    <div class="item" tabindex="0"
                        ...
                        <div class="item-icon">
                            <img class="size-16" src="/icons/parent-new.svg"/>
                        </div>
                        <div class="item-title" v-text="lang('createNewParent')"></div>
                    </div>
                </div>

                <div v-if="unSyncTabs.length && !showUnSyncTabs">
                    <hr>
                    <div class="item" tabindex="0" @click="showUnSyncTabs = true"
                         @keydown.enter="showUnSyncTabs = true">
                        <div class="item-icon">
                ...
                <div v-if="unSyncTabs.length && showUnSyncTabs" class="not-sync-tabs">
                    ...
                    <p class="h-margin-bottom-10">
                        ...
                        <ul>
                            <li>
                                <a tabindex="0" @click="unsyncHiddenTabsMoveToCurrentGroup"
                                   @keydown.enter="unsyncHiddenTabsMoveToCurrentGroup"
                                   v-text="lang('actionHiddenUnSyncTabsMoveAllTabsToCurrentGroup')"></a>
                            </li>
                            <li v-if="unSyncWindowTabs.length && countWindowsUnSyncTabs > 1">
                                <a tabindex="0" @click="unsyncHiddenWindowTabsCreateNewGroup"
                                   @keydown.enter="unsyncHiddenWindowTabsCreateNewGroup"
                                   v-text="lang('actionHiddenUnSyncWindowTabsCreateGroup')"></a>
                            </li>
                            <li>
                                <a tabindex="0" @click="unsyncHiddenTabsCreateNewGroupAll"
                                   @keydown.enter="unsyncHiddenTabsCreateNewGroupAll"
                                   v-text="lang('actionHiddenUnSyncTabsCreateGroup')"></a>
                            </li>
                            <li>
                                <a tabindex="0" @click="unsyncHiddenTabsCloseAll"
                                   @keydown.enter="unsyncHiddenTabsCloseAll"
                                   v-text="lang('actionHiddenUnSyncTabsCloseAll')"></a>
                            </li>
                        </ul>
                    </p>
                    <div>
            ...
            <div v-if="section === SECTION_GROUP_TABS" class="tabs-list">
                <div class="item is-unselectable" tabindex="0" @click="showSectionDefault"
                     @keydown.enter="showSectionDefault">
                ...
                <div class="group-info item no-hover mb-indent">
                    ...
                    <div class="item-action is-unselectable">
                        <span tabindex="0" @click="openGroupSettings(groupToShow)"
                              @keydown.enter="openGroupSettings(groupToShow)" class="size-16 cursor-pointer"
                              :title="lang('groupSettings')">
                            <img src="/icons/settings.svg"/>
                        </span>
                        <span tabindex="0" @click="removeGroup(groupToShow)"
                              @keydown.enter="removeGroup(groupToShow)" class="size-16 cursor-pointer"
                ...
                <template v-else>
                    ...
                    <div class="create-new-tab">
                        <div class="item" tabindex="0"
                             @contextmenu="$refs.contextMenuTabNew.open($event, {group: groupToShow})"
                             @click="addTab(groupToShow)" @keydown.enter="addTab(groupToShow)">
                            <div class="item-icon">
        ...
        <footer class="is-flex is-unselectable mt-indent">
            <div tabindex="0" class="is-flex is-align-items-center manage-groups is-full-height is-full-width"
                 @click="openManageGroups" @keydown.enter="openManageGroups" :title="lang('manageGroupsTitle')">
        ...
        <context-menu ref="settingsContextMenu">
            <ul class="is-unselectable">
                <li @click="showArchivedGroupsInPopup = !showArchivedGroupsInPopup">
                    <img :src="showArchivedGroupsInPopup ? '/icons/check-square.svg' : '/icons/square.svg'"
                         class="size-16"/>
